...
while (choice != 0)
{
    ...
    Console.WriteLine("5. Send mail with large attachment");
    ...
    switch(choice)
    {
        ...
        case 4:
            ...
            break;
        case 5:
            await SendMailWithLargeAttachmentAsync();
            break;
        default:
...
// </MakeGraphCallSnippet>
async Task SendMailWithLargeAttachmentAsync()
{
    var user = await GraphHelper.GetUserAsync();

    var userEmail = user?.Mail ?? user?.UserPrincipalName;
    await GraphHelper.SendMailWithLargeAttachmentAsync("Honkin file", "Sending you a really big file", userEmail, @"c:\users\2024425\downloads\24MiB.txt"); ;
}